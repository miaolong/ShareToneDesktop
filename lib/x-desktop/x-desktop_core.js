...
function xDesktop(respath,sdt,maxwin,_fmain) { 		// resourcepath, startupdesktop,maximumwindows,startup function
 ...
 var p_version_major = "1.5 modified by G. C. Lee";       // Major Release
 ...
 function _popupWindow(wName) {if (_property(wName,"wIndex") < p_syswin) return; if (! xDTwin.window(wName) || wName == p_lastpopup) return wName; _cbe(wName).zIndex(xDTwin.xDTdrag()); _property(wName,"zIndex",_cbe(wName).zIndex()); _show(wName); _property(wName,"wStat","OK"); p_lastpopup = wName; _taskbar(); return wName  }
 ...
 function _url(wName,wUrl,wScroll) {
  ...
  if (document.all) {e = document.getElementById('xDTiF_'+wName); e.src = wUrl; e.width = "100%"; e.height = "100%"; /*e.attachEvent('onload', function(){var xDT = parent.xDT;document.write('<link rel="stylesheet" type="text/css" href="' + xDT.resPath() + 'skins/' + xDT.dSkin() + '/' + xDT.dSkin() + '.css' + '">');});*/} 
 ...
 function _desktop() {
  ...
  function _skin(wSkin) {
   ...
   var uw = xDTwin.getAllWindows(p_syswin).split('/');
   for (var w in uw){
     //if (p_lastdt.length && p_lastdt != _property(uw[w],"wSkin") )
     //  continue;
     _refreshWindow(uw[w],wSkin);
     _positionWindow(uw[w]) 
   }
 ...
 function _taskbar(rWin) {
  ...
  for (var i=0;i<=maxwin;i++) { winName = xDTwin.wName(i); if (!winName) continue; if (i < p_syswin && !winName.match(/^(dMessage)$/) ) continue; if (_property(winName,"wStat") != "min") continue; if (str == "") str = winName; else str += "," + winName; }	
  wins = str.split(",");
  if (!str || wins.length < 1) {_cbe("dTaskbar").resizeTo(0,0); _cbe("dTaskbar").moveTo(0,0); _hide("dTaskbar"); p_taskbar = false; return }
  ...
  _cbe("dTaskbar").moveTo(0,document.cbe.height() - (++h) - 0 );
  _cbe("dTaskbar").moveTo(0,document.cbe.height() - (++h) - xDTwin.marginBottom(_dSkin()) );
 ...
 */
 ...
 function _calendar(calfield) {
  p_caldate = calfield; 	
  var w = 'mCalendar';
  _addWindow(w,'',228,220,'center','xDTnoBORDER');
